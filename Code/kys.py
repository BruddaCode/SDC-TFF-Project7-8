...
def platslaan(frame, src_points, dst_points):
    ...
    while True:
        if frameL is not None and frameR is not None:
            ...
            if not src_controls_ready:
                ...
                _create_point_trackbars(TRACKBAR_L_WINDOW, lw, lh, "Ls", defaultL)
                defaultDstL = np.array([
                    [0, 0],
                    [lw - 1, 0],
                    [0, lh - 1],
                    [lw - 1, lh - 1],
                ], dtype=np.float32)
                _create_point_trackbars(TRACKBAR_L_WINDOW, lw, lh, "Ld", defaultDstL)
                cv2.namedWindow(TRACKBAR_R_WINDOW, cv2.WINDOW_NORMAL)
                cv2.resizeWindow(TRACKBAR_R_WINDOW, 900, 320)
                _create_point_trackbars(TRACKBAR_R_WINDOW, rw, rh, "Rs", defaultR)
                defaultDstR = np.array([
                    [0, 0],
                    [rw - 1, 0],
                    [0, rh - 1],
                    [rw - 1, rh - 1],
                ], dtype=np.float32)
                _create_point_trackbars(TRACKBAR_R_WINDOW, rw, rh, "Rd", defaultDstR)
                src_controls_ready = True

            srcL = _read_points(TRACKBAR_L_WINDOW, "Ls")
            dstL = _read_points(TRACKBAR_L_WINDOW, "Ld")
            srcR = _read_points(TRACKBAR_R_WINDOW, "Rs")
