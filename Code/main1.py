...
if __name__ == "__main__":
    ...
    while True:
        ...
        # get latest intersections from each camera
        leftHit  = threadL.latestIntersection
        rightHit = threadR.latestIntersection
        # put current time into variable for PID controller
        currTime = time.time()

        # check where along a line off length 1 the middle of the lane is and set a mode for which lines where seen
        ...
        # set dt as the time since last PID compute
        dt = currTime - prevTime
        prevTime = currTime

        # compute the laneCenter to get a steering correction
        steer = pid.compute(laneCenter, dt)
        # transform the steering correction into a scale the carcontroller understands
        steer = -(round((np.clip(np.interp(steer, [-PID_STRENGTH, PID_STRENGTH], [-100, 100]), -100, 100)), 2))
        
        # using the modes set by the checkForHit function to check for broken lines
        # we know this is not a foolproof method, but with the time left this was what we could do
        if mode == "both":
            modes.append("both")
        ...
        # Logic that was meant to switch line upon entering the left direction on the t-crossing
        # This ultimately failed so the entire switching lanes when the lane ends needs some work
        ...
        # switching lane and setting the global variables since the switchLane function can't do it since it's not a class
        if switchToLeftLane:
        ...
        # checking to see if the code is not in debug mode
        # if it is and these functions are called, the code will stop because of an error
        ...
        # showing the edited camera frames on screen for debug reasons
        # best to turn off when demonstrating on the finale for some additional computing power
        ...
        # turn off the code when pressing q while one of the camera frames is selected
